...
class NotificationService {
  static async init({ apiKey, userId, onNotify, enablePush = false }) {
    ...
    // ── In-app (Reverb WebSocket) — optional ──────────────────────────
    if (config.reverb?.key) {
      try {
        ...
        console.warn("[NotificationService] Reverb unavailable, continuing:", reverbErr.message);
      }
    } else {
...
function waitForConnection(echo) {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(
      () => reject(new Error("[NotificationService] Connection timeout after 10s.")),
      10000
    );
  });
}
